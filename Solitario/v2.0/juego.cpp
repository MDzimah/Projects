...
bool Juego::salvar(std::ostream& salida) {
	...
	//Hay que restar 1 para "volver" al sistema de referencia (0,0) inicial del archivo leído
...
void Juego::posibles_movimientos(Movimiento& mov) const {
	...
	dirección respecto de la posición inicial para ver si están vacías.*/
	...
	for (int i = 0; i < 4; ++i) {
		...
		if (
			...
			(tablero.leer(posicionF, posicionC) == VACIA && tablero.leer(posicionF + dirs[i].first, posicionC + dirs[i].second) == NULA)
				//Como se muestra en el guión, hay dos formas de realizar un movimiento inverso
			)
		   )
		{
			posicion_valida(posicionF, posicionC);
